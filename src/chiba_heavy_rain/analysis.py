...
def _article_comparison_table(period_results: pd.DataFrame) -> pd.DataFrame:
    ...
    for station, article_location in ARTICLE_STATION_LABELS.items():
        ...
        rows.append(
            {
                "地点": station,
                "計算した再現期間（95%区間）": f"{estimate}（{lower}～{upper}）",
                "元記事の地域名": article_location.removeprefix("記事地域："),
                "元記事の再現期間": ARTICLE_RETURN_PERIODS[station],
            }
        )
...
def _write_report(
    ...
    text = f"""# 令和8年8月千葉豪雨：地上観測に基づく24時間雨量の再現期間
...
ただし、記事の574.2 mmは千葉市緑区～市原市東部の「解析雨量」（格子値）であり、地上観測所の値ではない。今回の地上観測解析は、その574.2 mm自体の再現期間を直接検証するものではなく、同じ豪雨を既存観測所で捉えた場合の局地ごとの頻度を示す。両者の空間代表性の違いを無視した一対一比較はできない。
...
| 記事の地域 | JMA観測所 | 使用期間 | 年最大値数 | 今回観測値 (mm) | 期間内最大 (mm) | 再現期間 |
|---|---|---:|---:|---:|---:|---:|
...
| 地点 | 計算した再現期間（95%区間） | 対応する元記事の地域名 | 元記事の再現期間 |
|---|---:|---|---:|
...
記事の解析雨量を代入した再現期間は、千葉で271年、茂原で115年となった。茂原では解析雨量がアメダス雨量より80.0 mm大きいため再現期間も長くなるが、それでも元記事の約277年より短い。千葉も元記事の約574年に対して約271年であり、雨量値の違いだけでは元記事との再現期間の差を説明できない。
...
| 地点 | 学習期間 | 今回24h (mm) | 既往最大 (mm) | 再現期間 | 95%ブートストラップ区間 | 2026含む |
|---|---:|---:|---:|---:|---:|---:|
{chr(10).join(rows)}

対象外：{excluded_text}

![地点別再現期間](return_period_summary.png)

![記事言及地域の最新系列GEV診断](return_level_diagnostics.png)

![期間による再現期間の比較](historical_period_comparison.png)
...
40～数十年の標本から数百～数千年以上へ外挿すると、GEV形状母数のわずかな違いで再現期間が桁違いになる。区間上限が「上限なし」の地点は、ブートストラップ標本の2.5%以上で生存確率が数値上ゼロまたは分布上端を超えたことを表す。したがって極端に長い点推定は精密な暦年数ではなく、「観測期間よりはるかに稀」という定性的証拠として扱うべきである。
